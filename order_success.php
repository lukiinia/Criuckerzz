...
// Ambil data pesanan
$sql_pesanan = "SELECT p.*, pl.nama_penerima, pl.alamat_lengkap, pl.nomor_hp, pm.metode_pembayaran, pm.status_pembayaran, pm.bukti_pembayaran 
                FROM pesanan p
                JOIN pelanggan pl ON p.id_pelanggan = pl.id_pelanggan
                JOIN pembayaran pm ON p.id_pesanan = pm.id_pesanan
                WHERE p.id_pesanan = ?";
$stmt = mysqli_prepare($conn, $sql_pesanan);
...
$sql_detail = "SELECT dp.*, pr.nama_produk, pr.foto
    ...
    <div class="breadcumb-wrapper" data-bg-src="">
      <div class="container">
        <div class="breadcumb-content">
          <h1 class="breadcumb-title">Status pesanan</h1>
    ...
    <div class="containerz">
            ...
            <?php foreach ($details as $detail): ?>
                <div class="product-detailsz">
                    <div style="display: flex; align-items: center;">
                        <img src="../adminfinish/uploads/<?= htmlspecialchars($detail['foto']) ?>" alt="<?= htmlspecialchars($detail['nama_produk']) ?>" alt="Minimalist Wristwatch">
                        <div class="product-infoz">
                            <h4><?= htmlspecialchars($detail['nama_produk']) ?></h4>
                            <p><span class="quantity"><?= $detail['quantity'] ?> ×</span> Rp<?= number_format($detail['total_harga'], 0, ',', '.') ?></p>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
        ...
        <div class="details-sectionz">
            <div>
                ...
                <p><?= htmlspecialchars($pesanan['alamat_lengkap']) ?></p>
                <p><?= htmlspecialchars($pesanan['nama_penerima']) ?> - <?= htmlspecialchars($pesanan['nomor_hp']) ?></p>
            </div>
            <div>
                <h4>Informasi Pembayaran</h4>
                <div class="payment-infoz">
                    <span class="visa-icon" style="margin-right: 10px;"><?= htmlspecialchars($pesanan['metode_pembayaran']) ?></span>
                    <p class="status-payment" style="color: green;">(<?= htmlspecialchars($pesanan['status_pembayaran']) ?>)</p>
                </div>
            </div>
            <div>
