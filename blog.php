  ...
  <div class="container">
    <div class="row">
      <div class="col-xxl-8 col-lg-7">
        ...
        $query = "SELECT * FROM blog WHERE status = 'active' ORDER BY created_at DESC";
        $result = mysqli_query($conn, $query);
        
        // Loop untuk menampilkan blog posts
        while ($row = mysqli_fetch_assoc($result)) {
          // Ambil data dari query
          $title = $row['title'];
          $content = substr($row['content'], 0, 200); // Ambil sebagian konten
          $image = $row['image'];
          $created_at = date('d F, Y', strtotime($row['created_at'])); // Format tanggal
        ?>
          <div class="th-blog blog-single">
            <div class="blog-img">
              <?php if ($image): ?>
                <a href="blog-details.php?id=<?= $row['id']; ?>">
                  <img src="../adminfinish/uploads/<?= $image; ?>" alt="Blog Image">
                </a>
              <?php endif; ?>
            </div>
            <div class="blog-content">
              <div class="blog-meta">
                <a class="author" href="blog.html">
                  <i class="far fa-user"></i>By Criuckerz
                </a>
                <a href="blog.php">
                  <i class="far fa-calendar"></i><?= $created_at; ?>
                </a>
                <a href="blog-details.php?id=<?= $row['id']; ?>">
                  <i class="far fa-comments"></i>Komentar (3)
                </a>
              </div>

              <h2 class="blog-title">
                <a href="blog-details.php?id=<?= $row['id']; ?>"><?= $title; ?></a>
              </h2>
              ...
              <a href="blog-details.php?id=<?= $row['id']; ?>" class="th-btn btn-sm">
        ...
        <div class="th-pagination text-center">
          <ul>
            ...
            <li>
              <a href="blog.php?page=next"><i class="far fa-arrow-right"></i></a>
            </li>
          </ul>
        </div>
      </div>
...
$query = "SELECT * FROM blog WHERE status = 'active' ORDER BY created_at DESC LIMIT 3";
$result = mysqli_query($conn, $query);
...
if (mysqli_num_rows($result) == 0) {
    echo "Tidak ada blog yang ditemukan.";
} else {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        <div class="recent-post">
            <div class="media-img">
                <a href="blog-details.php?id=' . $row['id'] . '">
                    <img src="../adminfinish/uploads/' . $image . '" alt="Gambar Blog" />
                </a>
            </div>
            <div class="media-body">
                <h4 class="post-title">
                    <a class="text-inherit" href="blog-details.php?id=' . $row['id'] . '">' . $title . '</a>
                </h4>
                <div class="recent-post-meta">
                    <a href="blog.html"><i class="far fa-calendar"></i>' . $created_at . '</a>
                </div>
            </div>
        </div>';
    }
