...
$subtotal = isset($_SESSION['cart']) ? array_sum(array_map(function ($item) {
    return $item['harga'] * $item['quantity'];
}, $_SESSION['cart'])) : 0;

$shippingCost = isset($_POST['shipping_method']) ? $_POST['shipping_method'] : 0;
...
<script>
  ...
  <div class="th-menu-area text-center">
    ...
    <input type="hidden" name="cart_data" value="<?= htmlspecialchars(json_encode($_SESSION['cart']), ENT_QUOTES, 'UTF-8') ?>">
        ...
        <div class="container">
            <!-- Bagian HTML yang sama -->
            <div class="woocommerce-notices-wrapper">
                <div class="woocommerce-message">Pesanan Anda</div>
            </div>
                        ...
                        <?php foreach ($_SESSION['cart'] as $item): ?>
                            <tr class="cart_item" data-id="<?= $item['id'] ?>">
                                ...
                                <td data-title="Remove">
                                    <a href="remove_from_cart.php?id_product=<?= $item['id'] ?>" class="remove"><i class="fal fa-trash-alt"></i></a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                    ...
                    <table class="cart_totals">
                        <tbody>
                            <tr>
                                <td>Subtotal</td>
                                <td>
                                    <span class="amount" id="total-subtotal">
                                        <bdi><span>Rp</span><?= number_format($subtotal, 0, ',', '.') ?></bdi>
                                    </span>
                                </td>
                            </tr>
                            <tr class="shipping">
                                <th>Pengiriman dan Penanganan</th>
                                <td>
                                <form action="deliver.php" method="POST">
    <ul class="woocommerce-shipping-methods list-unstyled">
        <li>
            <input type="radio" id="pengiriman_luar_jawa" name="shipping_method" value="20000" />
            <label for="pengiriman_luar_jawa">Luar Jawa (Rp 20.000)</label>
        </li>
        <li>
            <input type="radio" id="pengiriman_jawa" name="shipping_method" value="10000" checked />
            <label for="pengiriman_jawa">Jawa (Rp 10.000)</label>
        </li>
    </ul>
</form>
                        ...
                        <tfoot>
                            <tr>
                                <td>Total Pesanan</td>
                                <td>
                                    <strong><span class="amount" id="total-amount"><bdi><span>Rp</span><?= number_format($total, 0, ',', '.') ?></bdi></span></strong>
                                </td>
                            </tr>
                        </tfoot>
                    </table>
                    <div class="wc-proceed-to-checkout mb-30">
                    <button type="submit" class="th-btn">Lanjutkan Pesanan</button>                    </div>
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    // Fungsi untuk memperbarui total berdasarkan ongkir
    function updateTotal(shippingCost) {
        try {
            // Ambil subtotal dari elemen
            const subtotalText = document.getElementById('total-subtotal').innerText.trim();
            const subtotal = parseInt(subtotalText.replace(/[^\d]/g, ''), 10) || 0; // Konversi ke angka
            ...
            // Perbarui hidden input untuk form
            document.getElementById('subtotal-input').value = subtotal; // Input subtotal
            document.getElementById('ongkir-input').value = shippingCost; // Input ongkir
            document.getElementById('total-input').value = total; // Input total
        } catch (error) {
            console.error('Error updating total:', error);
        }
    }

    // Fungsi untuk inisialisasi nilai total awal
    function initializeTotal() {
        try {
            ...
            // Ambil metode pengiriman default yang dipilih
            const defaultShipping = document.querySelector('input[name="shipping_method"]:checked');
            const shippingCost = defaultShipping ? parseInt(defaultShipping.value, 10) || 0 : 0;
            ...
            // Perbarui hidden input awal
            document.getElementById('subtotal-input').value = subtotal;
            document.getElementById('ongkir-input').value = shippingCost;
            document.getElementById('total-input').value = total;
        } catch (error) {
            console.error('Error initializing total:', error);
        }
    }

    // Inisialisasi nilai total saat halaman selesai dimuat
    initializeTotal();

    // Pasang event listener untuk setiap opsi pengiriman
    const shippingOptions = document.querySelectorAll('input[name="shipping_method"]');
    shippingOptions.forEach(option => {
        option.addEventListener('change', (e) => {
            const shippingCost = parseInt(e.target.value, 10) || 0; // Ambil nilai ongkir dari opsi
            updateTotal(shippingCost); // Perbarui total
        });
    });
});
